...
export default {
    computed: {
        ...
        GetDepthofCut(){
            return this.FindParaNameIndex("Start X") * this.FindParaNameIndex("Start X") * Math.PI - (this.FindParaNameIndex("Start X") - this.FindParaNameIndex("Depth of cut")) * (this.FindParaNameIndex("Start X") - this.FindParaNameIndex("Depth of cut")) * Math.PI;
        },
	},
	data() {
		return {     
	...
	methods: {
        ...
        GetFlutes(){
            ...
            return parseFloat(this.ToolTurretPathDatas[this.toolstruct.datavalue].flutes, 10)
        },
        GetDiameter(){
            if(!this.ToolTurretPathDatas)return 0;
            return parseFloat(this.ToolTurretPathDatas[this.toolstruct.datavalue].Diameter, 10);
        },//
        GetWidthOfCut(){
            if(!this.ToolTurretPathDatas)return 0;
            return parseFloat(this.ToolTurretPathDatas[this.toolstruct.datavalue].WidthOfCut, 10);
        },
        GetFeedrate(){
        ...
        CreateGCode(){   
            ...
            for(let i=0 ; i< this.ThreadParameters.length ; i++ ){
                if(i==3 && this.isDoFinish){
                    g_code += " " + this.ThreadParameters[i].codetype + String(parseFloat(this.ThreadParameters[i].datavalue) + parseFloat(this.FinishingPass[0].datavalue));
                }else
                    g_code += " " + this.ThreadParameters[i].codetype + String(this.ThreadParameters[i].datavalue);
            }
            g_code += "\n";
            //
            if(this.isDoFinish){
                ...
                for(let i=0 ; i< this.ThreadParameters.length ; i++ ){
                    if(i==2){
                        g_code += " " + this.ThreadParameters[i].codetype + String(parseFloat(this.ThreadParameters[3].datavalue) + parseFloat(this.FinishingPass[0].datavalue));
                    }else
                        g_code += " " + this.ThreadParameters[i].codetype + String(this.ThreadParameters[i].datavalue);
        ...
        showcommon(){
            let strcom = ";***Below is Facing Cycle***\n";
            for(let i=0 ; i< this.ThreadParameters.length ; i++ ){
